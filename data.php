<?php

declare(strict_types=1);

$articles = [
	[
		"title" => "AIRCRAFT SKIDS OFF RUNWAY IN HEAVY MUMBAI RAINS",
		"image" => "/Images/airplane1.jpg",
		"content" => "An aircraft skidded off the runway at Mumbai airport when it landed in heavy rains.\n
                SpiceJet flight SG6237, which was flying from Jaipur in Rajasthan, overran the runway upon landing at Mumbai’s Chhatrapati Shivaji Maharaj airport.\n
                From images shared on social media, it appears the Boeing 737-800 jet slid through mud before coming to a stop off the side of the main runway at India’s second-largest airport.\n
                Images show the aircraft nose resting on the ground.\n
                It's thought that the airport is still working on extricating the aircraft. ",
		"published" => "2019-01-01",
		"likes" => "",
		"authorId" => 101
	],

	[
		"title" => "STRATOLAUNCH: WORLD'S LARGEST AIRCRAFT AND SATELLITE LAUNCH CONTENDER COMPLETES FIRST EVER FLIGHT",
		"image" => "/Images/airplane2.jpg",
		"content" => "A giant aircraft with the world’s longest wingspan has landed after its first ever flight.\n
                The twin-fuselage Stratolaunch jet landed two hours after taking off from Mojave Air and Space Port in California.\n
                The company said the jet hit altitudes of up to 17,000 feet (5,180 metres) above the Mojave Desert while performance and handling qualities were evaluated.\n
                CEO Jean Floyd said the aircraft made a “spectacular” landing, adding it was amazing to “watch this majestic bird take flight”.\n
                Established by the late Microsoft co-founder Paul Allen, Stratolaunch is vying to be a contender in the satellite launch market. Mr Allen died in October 2018.",
		"published" => "2019-01-02",
		"likes" => "",
		"authorId" => 101
	],

	[
		"title" => "Man builds homemade plane then uses road as runway to take off as hundreds watch in delight",
		"image" => "/Images/airplane3.jpg",
		"content" => "A man who was arrested by police in Pakistan for testing a homemade “flying machine” in front of a crowd of 500 without the approval of aviation authorities has asked to have the confiscated aircraft returned to him.\n
                Muhammad Fayaz, a popcorn seller and part-time security guard, said he was inspired to make the aeroplane after watching the National Geographic series Air Crash Investigation, and had sold a field, used his savings and taken out a bank loan to pay for it.\n
                The 92kg craft cost him a total of Rs50,000 (£270) to make and he claims it can fly to 1,000 feet.\n
                Mr Fayaz told reporters he had been told to contact Pakistan’s Civil Aviation Authority (CAA) to gain permission for his test flight, but he did not do so.\n
                “Owing to my limited resources, I was unable to contact the civil aviation officials and finally decided to take test flights without getting permission,” he told Pakistan’s Dawn.com.",
		"published" => "2019-02-12",
		"likes" => "",
		"authorId" => 202
	],

	[
		"title" => "Boeing jumbo jet tips backwards after airport workers mistakenly unload cargo from nose",
		"image" => "/Images/airplane4.jpg",
		"content" => "A 747 cargo jet suffered damage after it tipped backwards while parked on the tarmac when airport workers mistakenly unloaded the aircraft from its nose.\n
                The jet – belonging to Iranian cargo airline Fars Air Qeshm – became unbalanced during unloading at Doha airport in Qatar, tipping onto its tail.\n
                The Boeing 747-281, which had arrived from Yerevan, Armenia, is the airline's only aircraft.\n
                Incidents of aircraft tipping backwards are uncommon, but happen more frequently with cargo jets.\n
                Some airlines use tail stands when the aircraft are parked to prevent them tipping back.",
		"published" => "2019-08-23",
		"likes" => "",
		"authorId" => 202
	],

	[
		"title" => "THESE ARE THE WORLD'S SAFEST AIRCRAFTS",
		"image" => "/Images/airplane5.jpg",
		"content" => "Following last Sunday's Ethiopian Airlines crash, the safety of the modern Boeing 737 MAX aircraft has been brought into question.\n
        After the crash, which killed all 157 people onboard, the entire fleet of 737 MAXs have been grounded.\n
        The 737 MAX aircraft has now suffered two fatal crashes in less than five months: the plane was also involved in the October 2018 Lion Air accident, which killed all 189 onboard.\n
        In total, 346 people have died onboard a Boeing 737 MAX.\n
        According to a Boeing report from October 2018, there are nine commercial aircraft that have never recorded a passenger fatality: Boeing 717, CRJ700/900/1000, Airbus A380, Boeing 787, Boeing 747-8, Airbus A350, Bombardier C Series, A340 and the Airbus A320 NEO series.",
		"published" => "2019-12-24",
		"likes" => "",
		"authorId" => 303
	],

	[
		"title" => "Lion Air crash: Divers and underwater robots deployed after 189 killed in Indonesia plane disaster",
		"image" => "/Images/airplane6.jpg",
		"content" => "World leaders and the Pope sent their condolences to bereaved families in Indonesia after a passenger plane plunged into the sea off Indonesia soon after take-off, killing all 189 people on board. \n
        An investigation has been launched after the budget-airline jet, which was new but had experienced a “technical issue” on its previous flight, came down near Jakarta.\n
        Rescuers in inflatable boats had retrieved 10 intact bodies, various body parts, pieces of aircraft and personal belongings from the ocean.\n
        They worked through the night, sending 24 body bags to identification experts while the airline flew dozens of grieving relatives to the country’s capital.",
		"published" => "2019-08-23",
		"likes" => "",
		"authorId" => 303
	],

	[
		"title" => "BOEING SET TO START TESTING SELF-FLYING PLANES",
		"image" => "/Images/airplane7.jpg",
		"content" => "Boeing is set to start testing self-flying planes, the company has revealed.\n
            It says it wants to develop autonomous aircraft, which would be capable of navigating themselves without any input from a human pilot.\n
            Planes can already take off, cruise and land with minimal human assistance, but Boeing wants to go a step further.",
		"published" => "2017-09-03",
		"likes" => "",
		"authorId" => 404
	],

	[
		"title" => "THE 'VVIP' PRIVATE JET THAT COSTS £20,000 AN HOUR TO HIRE",
		"image" => "/Images/airplane8.jpg",
		"content" => "With a master bedroom, a tablet-controlled widescreen TV and domed ceilings, it’s clear why the Dreamliner Boeing 787 airliner has earned its label of a ‘VVIP' aircraft. \n
            Believed to be the world’s largest private jet, the remodelled B787 Dreamliner costs around £20,000 per hour to hire. An 11-hour flight from London to Los Angeles would set back a client at least £242,000.",
		"published" => "2016-09-08",
		"likes" => "",
		"authorId" => 404
	],

	[
		"title" => "A LUXURY CRUISE SHIP IN THE SKY? WHAT FLYING WILL LOOK LIKE IN THE FUTURE ACCORDING TO A CABIN DESIGNER",
		"image" => "/Images/airplane9.jpg",
		"content" => "The mid-twentieth century is sometimes referred to as the golden age of flying. Air travel was an event, where – depending on the airline – planes had piano bars, in-flight sommeliers, and waiters in white suits who dished out caviar. Now, airlines are ditching free food altogether and nightmarish diagrams of cabins where passengers are packed in like sardines don’t bode well for the future of air travel. \n
                But air cabin designers aren’t the bad guys, argues David Kondo, the manager of cabin interior development at Finnair. He insists they’re up not only against budgets, but also the laws of physics when it comes to making air travel pleasant for passengers. ",
		"published" => "2012-12-12",
		"likes" => "",
		"authorId" => 505
	],

	[
		"title" => "BOOM SUPERSONIC JET ABLE TO FLY LONDON TO NEW YORK IN THREE HOURS A STEP CLOSER TO PRODUCTION",
		"image" => "/Images/airplane10.jpg",
		"content" => "A supersonic jet capable of flying passengers from London to New York in three hours and 15 minutes has moved a step closer to reality.  \n
                Boom Supersonic has raised $33m (£26m) in Series A funding, enough to build and fly the prototype XB-1.\n
                Nicknamed “Baby Boom”, the test model will be a third of the size of the jets it eventually plans to sell to airlines.\n
                “We have almost all the engineering completed, and the first wing components are showing up in the office this week,” Boom CEO Blake Scholl told TechCrunch. ",
		"published" => "2022-12-23",
		"likes" => "",
		"authorId" => 505
	]
];

$authors = [
	101 => [
		"name" => "Michael Scott",
		"authorImage" => "/Images/michaelscott.jpeg"
	],

	202 => [
		"name" => "Jim Halpert",
		"authorImage" => "/Images/jimhalpert.jpeg"
	],

	303 => [
		"name" => "Dwight Schrute",
		"authorImage" => "/Images/dwightschrute.jpeg"
	],

	404 => [
		"name" => "Pam Beesly",
		"authorImage" => "/Images/pambeesly.jpeg"
	],

	505 => [
		"name" => "Kevin Malone",
		"authorImage" => "/Images/kevinmalone.jpeg"
	]
];
